...
defmodule Act do
 ...
 @doc """
  ...
  def rotate_left(list, number) do
    rotate_left(list, number, [])
  end

  defp rotate_left(list, 0, result) do
    result ++ list
  end

  defp rotate_left(list, number, result) do
    if number > 0 do
      rotate_left(tl(list) ++ [hd(list)], number - 1, result)
    else
      rotate_left([last(list)] ++ init(list), number + 1, result)
    end
  end
